...
//                LogUtils.e("surfaceCreated Player setVideoWindow failed:" + Player.getInstance().getLastError(m_iPort));
//            } else {
//                LogUtils.e("surfaceCreated Player setVideoWindow succeed");
//            }
    ...
    public boolean init() {
        ...
        if (!result) {
            LogUtils.e("HCNetSDK init is failed! error code:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
            return false;
        }
    ...
    private int loginNormalDevice() {
        ...
        String m_oPort = loginInfo.m_oPort;
        if (TextUtils.isEmpty(m_oPort)) {
            m_oPort = "8000";
        }
        ...
        if (iLogID < 0) {
            LogUtils.e("NET_DVR_Login is failed!Err:"
        ...
        LogUtils.i("NET_DVR_Login is Successful!");
    ...
    private ExceptionCallBack getExceptiongCbf() {
        ExceptionCallBack oExceptionCbf = new ExceptionCallBack() {
            public void fExceptionCallBack(int iType, int iUserID, int iHandle) {
                LogUtils.e("recv exception, type:" + iType);
            }
        };
    ...
    public boolean login() {
        if (m_iLogID < 0) {
            ...
            if (!HCNetSDK.getInstance().NET_DVR_SetExceptionCallBack(
                    oexceptionCbf)) {
                LogUtils.e("NET_DVR_SetExceptionCallBack is failed! error code:", HCNetSDK.getInstance().NET_DVR_GetLastError());
                return false;
            }
    ...
    public boolean logout() {
        ...
        if (!HCNetSDK.getInstance().NET_DVR_Logout_V30(m_iLogID)) {
            LogUtils.e(" NET_DVR_Logout is failed!, error code" + HCNetSDK.getInstance().NET_DVR_GetLastError());
            return true;
        }
        m_iLogID = -1;
        return false;
    ...
    public String getImg() {
        if (m_iLogID < 0) {
            LogUtils.e("startPreviewInternal error please login on device first");
            return "";
        }

        File dir = new File(FileUtils.getDiskCacheDir(applicationContext) + "Screenshot/");
        if (!dir.exists()) {
            dir.mkdir();
        }
        if (sDateFormat == null) {
        ...
        String filepath = dir.getAbsolutePath() + "snap_" + Constant.getDeviceId(applicationContext) + "_" + date + ".jpg";

        NET_DVR_JPEGPARA lpJpegPara = new NET_DVR_JPEGPARA();
        lpJpegPara.wPicQuality = 0;
        lpJpegPara.wPicSize = 5;
        boolean b = HCNetSDK.getInstance().NET_DVR_CaptureJPEGPicture(m_iLogID, 1, lpJpegPara, filepath);

        if (!b) {
            LogUtils.e("NET_DVR_RealPlay is failed!Err:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
            return "";
        }

        LogUtils.i("NET_DVR_RealPlay success");
        return filepath;
    }
    ...
    public void startSinglePreview() {
        if (surfaceView == null || surfaceView.get() == null) {
            LogUtils.e("must call setSurfaceView before preview");
            return;
        }
    ...
    private boolean startPreviewInternal() {
        ...
        if (m_iPlaybackID >= 0) {
            LogUtils.e("startPreviewInternal Please stop palyback first");
            return false;
        }
        RealPlayCallBack fRealDataCallBack = getRealPlayerCbf();
        LogUtils.i("m_iStartChan:" + m_iStartChan);
    ...
    public boolean stopSinglePreview() {
        if (m_iPlayID < 0) {
            LogUtils.e("m_iPlayID < 0");
            return false;
        }

        // net sdk stop preview
        if (!HCNetSDK.getInstance().NET_DVR_StopRealPlay(m_iPlayID)) {
            LogUtils.e("StopRealPlay is failed!Err:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
            return false;
        }
    ...
    private void stopSinglePlayer() {
        ...
        if (!Player.getInstance().stop(m_iPort)) {
            LogUtils.e("stop is failed!");
            return;
        }

        if (!Player.getInstance().closeStream(m_iPort)) {
            LogUtils.e("closeStream is failed!");
            return;
        }
        if (!Player.getInstance().freePort(m_iPort)) {
            LogUtils.e("freePort is failed!" + m_iPort);
            return;
        }
    ...
    public void processRealData(int iPlayViewNo, int iDataType,
        ...
        if (HCNetSDK.NET_DVR_SYSHEAD == iDataType) {
            ...
            if (m_iPort == -1) {
                LogUtils.e("getPort is failed with: " + Player.getInstance().getLastError(m_iPort));
                return;
            }
            LogUtils.i("getPort succ with: " + m_iPort);
            if (iDataSize > 0) {
                if (!Player.getInstance().setStreamOpenMode(m_iPort, iStreamMode)) {// set stream mode
                    LogUtils.e("setStreamOpenMode failed");
                    return;
                }
                if (!Player.getInstance().openStream(m_iPort, pDataBuffer, iDataSize, 2 * 1024 * 1024)) {// open stream
                    LogUtils.e("openStream failed");
                    return;
                }
                ...
                if (!Player.getInstance().playSound(m_iPort)) {
                    LogUtils.e("playSound failed with error code:" + Player.getInstance().getLastError(m_iPort));
                }
...
//                        LogUtils.i(TAG, "onDisplay:" + bytes.length);
                        if (seeking) {
                            seeking = false;
                            notifySeekComplete();
                        }
...
//                });
            ...
            if (!Player.getInstance().inputData(m_iPort, pDataBuffer, iDataSize)) {
                LogUtils.v("inputData failed with: " +
...
//                }
    ...
    public void playBack(FileInfo fi) {
        ...
        fileInfo = fi;
        if (surfaceView == null || surfaceView.get() == null) {
            LogUtils.e("playback must call setSurfaceView before preview");
            return;
        }
    ...
    private void playBackInternal(String fileName) {
        if (m_iLogID < 0) {
            LogUtils.e("playBackInternal please login on a device first");
            return;
        }
        if (m_iPlaybackID < 0) {
            if (m_iPlayID >= 0) {
                LogUtils.i("playBackInternal Please stop preview first");
                return;
            }
            ...
            if (m_iPlaybackID >= 0) {
                ...
                LogUtils.i("NET_DVR_PlayBackByName failed, error code: " +
                        HCNetSDK.getInstance().NET_DVR_GetLastError());
            }
        }
    }
    ...
    public void pausePlayBack() {
        if (!HCNetSDK.getInstance().NET_DVR_PlayBackControl_V40(m_iPlaybackID,
            ...
            LogUtils.e("net sdk playback pause failed! error code:", HCNetSDK.getInstance().NET_DVR_GetLastError());
        }
    ...
    public void resumePlayBack() {
        if (!HCNetSDK.getInstance().NET_DVR_PlayBackControl_V40(m_iPlaybackID,
            ...
            LogUtils.e("net sdk playback resume failed! error code:", HCNetSDK.getInstance().NET_DVR_GetLastError());
        }
    ...
    public void playbackSeekTo(float percent) {
        ...
        if (!HCNetSDK.getInstance().NET_DVR_PlayBackControl_V40(m_iPlaybackID,
                12, bytes, 0, null)) {
            LogUtils.e("NET_DVR_PlayBackControl_V40 seek failed, error code: " +
                    HCNetSDK.getInstance().NET_DVR_GetLastError());
        } else {
            LogUtils.i("NET_DVR_PlayBackControl_V40 seek success");
        }
    ...
    public void stopPlayback() {
        ...
        if (!HCNetSDK.getInstance().NET_DVR_StopPlayBack(m_iPlaybackID)) {
            LogUtils.e("net sdk stop playback failed! error code:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
        } // player stop play
        stopSinglePlayer();
        notifyPlayStop();

//        if (!Player.getInstance().setVideoWindow(m_iPort, 0, null)) {
//            LogUtils.e("stopPlayback Player setVideoWindow failed: " + Player.getInstance().getLastError(m_iPort));
//        }
        m_iPlaybackID = -1;
    }
    ...
    public List<FileInfo> findFile(TimeStruct startTime, TimeStruct stopTime) {
        if (m_iLogID < 0) {
            LogUtils.e("findFile please login on a device first");
...
//        LogUtils.i(TAG, "Player getCurrentFrameNum====" + current);
//        LogUtils.i(TAG, "Player preFrameNumber====" + preFrameNumber);
//
//        LogUtils.i(TAG, "Player Frame isEnd====" + (current == preFrameNumber));
    ...
    public int getPlayBackTime() {
        ...
        LogUtils.i("Player getPlayedTime====" + Player.getInstance().getPlayedTime(m_iPort));
        return progress;
    }
    ...
    public int getPlaybackDuration(FileInfo fileInfo) {
        if (fileInfo != null) {
            ...
            LogUtils.i("getPlaybackDuration ==== " + duration);
            return duration;
        }
        return 0;
    }
    ...
    public void focusFar() {
        if (!HCNetSDK.getInstance().NET_DVR_PTZControl(m_iPlayID, PTZCommand.FOCUS_FAR, 0)) {
            LogUtils.e("PTZControl  PAN_LEFT 0 faild!" + " err: " + HCNetSDK.getInstance().NET_DVR_GetLastError());
        } else {
            LogUtils.i("PTZControl  PAN_LEFT 0 succ");
        }
        if (!HCNetSDK.getInstance().NET_DVR_PTZControl(m_iPlayID, PTZCommand.FOCUS_FAR, 1)) {
            LogUtils.e("PTZControl  PAN_LEFT 1 faild!" + " err: " + HCNetSDK.getInstance().NET_DVR_GetLastError());
        } else {
            LogUtils.i("PTZControl  PAN_LEFT 1 succ");
        }
    }
    ...
    public void startRecord() {
        ...
        if (!HCNetSDK.getInstance().NET_DVR_StartDVRRecord(m_iLogID, 1, 0)) {
            LogUtils.e("NET_DVR_StartDVRRecord err:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
        } else {
            LogUtils.i("NET_DVR_StartDVRRecord succ!");
            recording = true;
        }
    }
    ...
    public void stopRecord() {
        ...
        if (!HCNetSDK.getInstance().NET_DVR_StopDVRRecord(m_iLogID, 1)) {
            LogUtils.i("NET_DVR_StopDVRRecord err:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
        } else {
            LogUtils.i("NET_DVR_StopDVRRecord succ!");
            recording = false;
        }
    }
}
