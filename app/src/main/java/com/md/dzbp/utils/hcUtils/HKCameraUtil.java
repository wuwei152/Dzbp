...
public class HKCameraUtil {
    ...
    public static List<FileInfo> findFile(int iUserID, TimeStruct start, TimeStruct stop) {
        ...
        LogUtils.i("查找文件开始时间：" + start.toString() + " 结束时间：" + stop.toString());
        iFindHandle = HCNetSDK.getInstance().NET_DVR_FindFile_V30(iUserID, lpSearchInfo);
        if (iFindHandle == -1) {
            LogUtils.e("NET_DVR_FindFile_V30 failed,Error:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
            return list;
        }
        int findNext = 0;
        ...
        while (findNext != -1) {
            ...
            if (findNext == HCNetSDK.NET_DVR_FILE_SUCCESS) {
                ...
                list.add(fileInfo);
                LogUtils.i("~~~~~Find File" + fileName + "~~~~~File Size" + struFindData.dwFileSize +
    ...
    public static void testGetCompressCfg(int playid, int channel) {
        ...
        if (HCNetSDK.getInstance().NET_DVR_GetDVRConfig(playid, HCNetSDK.NET_DVR_GET_COMPRESSCFG_V30, channel, Compress_Cfg)) {
			...
			*/
            LogUtils.i("Audio Enc Type =	" + net_para.byAudioEncType);                    //0-G722	1-G711_U	2-G711_A	5-MP2L2	6-G726	7-AAC，0xfe- 自动（和源一致），0xff-无效
            LogUtils.i("Bit Rate Type = 	" + net_para.byBitrateType);                    //0-变码率	1-定码率
            LogUtils.i("Frame Type = 		" + net_para.byIntervalBPFrame);                //0-BBP帧	1-BP帧 	2-P帧	0xff-无效
            LogUtils.i("Pic Qua = 			" + net_para.byPicQuality);                    //0 最高————>5最低		0xfe-自动(和源一致)
            LogUtils.i("Resolution = 		" + net_para.byResolution);                    //太多了，写不下，用到的时候查文档吧
            LogUtils.i("Stream Type = 		" + net_para.byStreamType);                    //0-视屏流	1-复合流	0xfe-自动(和源一致)
            LogUtils.i("Video Enc Type = 	" + net_para.byVideoEncType);                    //0-私有264，1-标准h264，2-标准mpeg4，7-M-JPEG，8-MPEG2，0xfe- 自动（和源一致），0xff-无效
            LogUtils.i("Video Bit Rate		" + net_para.dwVideoBitrate);                    //视频码率		太多不写	请查询文档
            LogUtils.i("Video Frame Rate = 	" + net_para.dwVideoFrameRate);                //视屏帧率		0-全部	1-1/16.....查文档
            LogUtils.i("IntervalFrameI = 	" + net_para.wIntervalFrameI);                //I 帧间隔	0xffee	和源一致	0xffff	无效
            channel = 0xFFFFFFFF;
//			Compress_Cfg = null;
        } else {
            LogUtils.e("获取视频参数失败,Error:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
        }
    ...
    public static void testGetAbility(int userId, int channel) {
        ...
        if (HCNetSDK.getInstance().NET_DVR_GetCompressionAbility(userId, channel, Compress_Cfg)) {
            LogUtils.e("NET_DVR_GetCompressionAbility,Success:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
        } else {
            LogUtils.e("NET_DVR_GetCompressionAbility,Error:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
        }
